...
async function saveAsImage() {
	...
	link.download = "billionaire-face-match.png"
	link.click()
}
...
// 고정 문구("Meet my billionaire face match!")보다 실제 결과가 들어간 문구가 클릭률이 훨씬
...
function buildShareText() {
	...
	if (topMatchName && archetypeName) {
		return `My match is ${topMatchName}: ${topSimilarity}% similarity to an AI caricature! My standout feature: ${archetypeName}. Just for fun.`
	}
	if (archetypeName) {
		return `My standout feature is ${archetypeName}! Try your own billionaire face match.`
	}
...
async function shareCardOrDownload(fallbackMessage) {
	...
	const file = new File([blob], "billionaire-face-match.png", { type: "image/png" })

	if (navigator.canShare && navigator.canShare({ files: [file] })) {
		try {
			await navigator.share({
				files: [file],
				title: "Billionaire Face Match",
				text: buildShareText(),
			})
			return
		} catch (err) {
...
document.getElementById("webShareBtn").addEventListener("click", async function () {
	...
	await shareCardOrDownload("Your card was downloaded. Attach it in your favorite app to share.")
})
...
document.getElementById("instagramShareBtn").addEventListener("click", async function () {
	...
	await shareCardOrDownload("Your card was downloaded. Open Instagram and select the saved image to share.")
})
